...
def _config():
    return ResearchTrainingConfig(
        ...
        sample_count=8,
        validation_count=8,
        source_prefit_count=8,
        initial_training_rank=2,
        max_network_depth=1,
    )


def test_high_rank_capacity_is_three_layer_response_funnel():
    depth, channels, linear, hidden, quadratic, square, cross, state = _default_capacity(198, 12)
    assert (depth, channels) == (3, 1)
    assert (linear, quadratic, square) == (8, 16, 8)
    assert (hidden, cross, state) == (6, 6, 4)
    widths, hidden_by_layer, cross_by_layer, state_by_layer = _default_layer_architecture(198, 3)
    assert widths == (198, 64, 32)
    assert hidden_by_layer == (6, 4)
    assert cross_by_layer == (6, 4)
    assert state_by_layer == (4, 2)
...
def test_progressive_acceptance_does_not_freeze_one_already_good_point():
    old = np.array([1e-7, 0.04, 0.03])
    new = np.array([2e-5, 0.032, 0.024])
    assert _accept_progressive(old, new, 1e-5, np.ones_like(old))
