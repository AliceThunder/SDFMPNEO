...
from sdfmpneo.training.automatic_thermal_rank import (
    _load_thermal_rank_cache,
    _save_thermal_rank_cache,
    _thermal_rank_cache_key,
...
def test_thermal_rank_cache_round_trip_and_mesh_invalidation(tmp_path):
    config_path, config = _cache_config(tmp_path)
    key = _thermal_rank_cache_key(config_path, config)
    ...
    _save_thermal_rank_cache(cache_path, key, 198, report)
    rank, cached = _load_thermal_rank_cache(cache_path, key)
    assert rank == 198
    assert cached["cache_hit"] is True
    assert Path(cached["cache_path"]) == cache_path

    (tmp_path / "mesh.msh").write_bytes(b"changed mesh")
    changed_key = _thermal_rank_cache_key(config_path, config)
    assert changed_key != key
    assert _load_thermal_rank_cache(cache_path, changed_key) is None


def test_thermal_rank_cache_invalidates_when_physics_changes(tmp_path):
    config_path, config = _cache_config(tmp_path)
    original = _thermal_rank_cache_key(config_path, config)
    config["materials"]["1"]["thermal_conductivity"] = 4.0
    assert _thermal_rank_cache_key(config_path, config) != original
