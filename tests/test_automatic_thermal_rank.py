...
def test_automatic_restart_box_uses_physical_response_envelope():
    training = {
        "initial_lower": [], "initial_upper": [],
        "operating_lower": [0.0], "operating_upper": [1.0],
    }
    report = {"recommended_restart_coordinate_bound": [0.3, 0.2, 0.12]}
    resolved = resolve_training_bounds(
        training, 3, {"initial_coordinate_bound": 0.1}, rank_report=report
    )
    assert resolved["initial_lower"] == [-0.3, -0.2, -0.12]
    assert resolved["initial_upper"] == [0.3, 0.2, 0.12]


def test_automatic_initial_box_falls_back_to_configured_bound_without_report():
    training = {
        "initial_lower": [], "initial_upper": [],
        "operating_lower": [0.0], "operating_upper": [1.0],
    }
...
def test_explicit_initial_box_must_match_selected_rank():
    with pytest.raises(ValueError, match="thermal rank selected 3"):
        resolve_training_bounds(
            {"initial_lower": [-1, -1], "initial_upper": [1, 1]}, 3, {}
        )
