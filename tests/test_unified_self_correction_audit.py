...
def _local(step, *, joule_error=0.0, residual=0.0, linear_converged=True):
    step = float(step)
    # Raw longitudinal/terminal response is deliberately non-convergent. The
    # canonical local correction is allowed to certify only its localized part.
...
def test_local_self_audit_gates_localized_defect_not_raw_terminal_response(monkeypatch):
    ...
    # This is the production situation observed in the real preflight: raw
    # longitudinal response may be catastrophic while localized defect converges.
