...
def _multi_source_graph():
    ...
    graph.add_response_state(
        "h0", 0,
        ((("u0",), 0.6), (("u1",), -0.25), (("u0", "u1"), 0.12)),
    )
    graph.add_response_state(
        "h1", 1,
        ((("h0", "u0"), 0.35), (("h0", "u1"), -0.18)),
    )
...
def test_exact_multi_source_weight_jacobian_matches_central_difference():
    ...
    weights = np.array([
        source.weight.real for node in graph.response_nodes
...
def test_legacy_single_source_with_multiple_response_parents_stays_supported():
    ...
    for time in (0.03, 0.8, 50.0, np.inf):
        expected = evaluate_state_stable(
            graph, time, a0=[0.1, -0.05], operating=[1.2])
...
def test_fixed_research_checkpoint_roundtrip_keeps_multi_source_states(tmp_path):
    ...
    for time in (0.0, 0.2, 1000.0, np.inf):
        ...
        actual = loaded.predict(
            time, a0=[0.4], operating=[20.0, 8.0], diagnostics=False)
        assert np.allclose(
            actual["thermal_coordinates"], expected["thermal_coordinates"],
            rtol=2e-12, atol=2e-13)
        assert np.allclose(
            actual["thermal_derivative"], expected["thermal_derivative"],
            rtol=2e-12, atol=2e-13)


def test_residual_driven_state_runtime_remains_available_for_legacy_dag_resume():
    ...
    from sdfmpneo.training.residual_state_runtime import (
        residual_driven_state_train, state_refine_weights)

    assert callable(residual_driven_state_train)
    assert callable(state_refine_weights)
