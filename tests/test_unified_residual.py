...
def test_sparse_graph_retains_signed_complex_offdiagonal_coupling():
...
def test_network_outputs_full_edge_correction_not_global_coefficients():
    torch = pytest.importorskip("torch")
    A, B = random_problem(seed=9)
    network = build_edge_residual_operator(
        TinyTopology(), {"width": 8, "message_passing_steps": 2, "activation": "silu"}
    ).double()
    graph = operator_feature_statistics(A)
    features, _, _ = residual_features(B, graph)
    coupling = graph.torch_coupling(torch, "cpu", torch.float64)
    output = network(torch.as_tensor(features, dtype=torch.float64), coupling)
...
def test_zero_initialized_network_fallback_then_fgmres_closes_true_residual():
    ...
    network = build_edge_residual_operator(
        TinyTopology(), {"width": 8, "message_passing_steps": 1, "activation": "silu"}
    ).double()
