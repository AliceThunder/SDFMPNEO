...
def test_neural_guess_is_only_an_initial_guess_and_true_residual_controls_answer():
    ...
    class ZeroGuess(torch.nn.Module):
        def __init__(self):
            super().__init__()
            self.anchor = torch.nn.Parameter(torch.zeros(1, dtype=torch.float64))
        def forward(self, x):
            return torch.zeros((x.shape[0], output_dimension), dtype=x.dtype, device=x.device) + 0.0 * self.anchor

    network = ZeroGuess().double()
    accelerator = NeuralMaxwellAccelerator(network, V, residual_tolerance=1e-10, max_iterations=100)
