...
def test_global_solver_uses_bounded_sparse_direct_correctness_fallback(monkeypatch):
    class Background:
        background_config = {
            "linear_solver": {
                "relative_residual_tolerance": 1e-10,
                "direct_max_dofs": 1,
                "direct_fallback_max_dofs": 8,
                "iterative_maxiter": 1,
                "iterative_inner_m": 2,
                "defect_steps": 0,
                "ilu_drop_tolerance": 5e-3,
    ...
    monkeypatch.setattr(
        fast_global,
        "build_transverse_ilu",
        lambda *args, **kwargs: (Preconditioner(), {}),
    )
    monkeypatch.setattr(
        fast_global,
        "compose_block_preconditioner",
    ...
    def fake_direct(A, B):
        calls["shape"] = A.shape
        X = np.column_stack(
            [np.asarray(spla.spsolve(A, B[:, p]), complex) for p in range(B.shape[1])]
        )
        return X, 0.01

    monkeypatch.setattr(fast_global, "_direct", fake_direct)
    ...
    assert labels[-1] == "sparse-direct-correctness-fallback"


def test_global_direct_fallback_default_excludes_large_local_validation_systems():
    class Background:
        background_config = {"linear_solver": {}}

    cfg = fast_global._cfg(Background())
    assert cfg["direct_max_dofs"] == 60000
    assert cfg["direct_fallback_max_dofs"] == 100000
    assert cfg["direct_fallback_max_dofs"] < 118000



def test_global_direct_fallback_failure_continues_to_shifted_recovery(monkeypatch):
    class Background:
        background_config = {
            "linear_solver": {
                "relative_residual_tolerance": 1e-10,
    ...
    monkeypatch.setattr(
        fast_global,
        "_direct",
        lambda A, B: (_ for _ in ()).throw(MemoryError("synthetic LU failure")),
    )
    ...
    assert np.allclose(X, truth)
    labels = [row["solver"] for row in history]
    assert "sparse-direct-correctness-fallback" in labels
    fallback = next(
        row for row in history
        if row["solver"] == "sparse-direct-correctness-fallback"
    )
    assert fallback["preconditioner_failed"] is True
    assert labels[-1] == "compatible-shifted-ilu-tight-recovery"


def test_global_solver_prefers_defect_cleanup_before_direct_fallback(monkeypatch):
    class Background:
        background_config = {
            "linear_solver": {
                "relative_residual_tolerance": 1e-10,
    ...
    class LocalSolver:
        @staticmethod
        def _ilu_preconditioner(A, *, drop_tol, fill_factor, shift_factor):
            raise AssertionError("shifted fallback should not be needed")

        @staticmethod
        def _lgmres(A, rhs, *, x0, M, rtol, maxiter, inner_m):
            exact = np.asarray(spla.spsolve(A, rhs), complex)
            return 0.995 * exact, maxiter
    ...
    def forbidden_direct(A, B):
        raise AssertionError("near-converged solve should use defect cleanup first")

    monkeypatch.setattr(fast_global, "_direct", forbidden_direct)
    ...
    def exact_cleanup(
        ...
        *,
        ...
        residual = float(np.max(fast_global._true_residuals(A, exact, B)))
        return exact, residual, ({
            "solver": "compatible-transverse-defect-1",
            "maximum_relative_residual": residual,
        },)

    monkeypatch.setattr(fast_global, "_defect_cleanup", exact_cleanup)
    ...
    assert np.allclose(X, truth)
    labels = [row["solver"] for row in history]
    assert "sparse-direct-correctness-fallback" not in labels
    assert labels[-1] == "compatible-transverse-defect-1"
