...
def test_global_solver_defect_rescue_uses_true_residual_after_primary_stall(monkeypatch):
    class Background:
        background_config = {
            "linear_solver": {
                "relative_residual_tolerance": 1e-10,
                ...
                "iterative_inner_m": 4,
                "defect_steps": 0,
                "defect_rescue_start_residual": 2e-2,
                "defect_rescue_steps": 4,
                "defect_rescue_maxiter": 24,
                "defect_rescue_inner_m": 30,
                "ilu_drop_tolerance": 5e-3,
                "ilu_fill_factor": 4.0,
                "ilu_strong_drop_tolerance": 1e-3,
                "ilu_strong_fill_factor": 8.0,
    ...
    def fake_cleanup(
        ...
        *,
        ...
        seen.update(
            ...
            incoming=float(np.max(fast_global._true_residuals(A, X, B))),
        )
        ...
        return exact, residual, ({
            "solver": f"{label_prefix}-1",
            "maximum_relative_residual": residual,
        },)

    monkeypatch.setattr(fast_global, "_defect_cleanup", fake_cleanup)
    ...
    assert seen["incoming"] <= 2e-2
    assert seen["steps"] == 4
    assert seen["maxiter"] == 24
    assert seen["inner_m"] == 30
    assert seen["label_prefix"] == "certified-defect-rescue"
    assert residual <= 1e-10
    assert np.allclose(X, truth)
    assert history[-1]["solver"] == "certified-defect-rescue-1"


def test_global_solver_defect_rescue_defaults_cover_observed_hard_geometry_basin():
    class Background:
        background_config = {
            "linear_solver": {
                "iterative_defect_steps": 3,
...
def test_global_solver_uses_bounded_direct_correctness_fallback(monkeypatch):
    class Background:
        background_config = {
            "linear_solver": {
                "relative_residual_tolerance": 1e-10,
    ...
    assert residual <= 1e-10
    assert np.allclose(X, truth)
    assert history[-1]["solver"] == "sparse-direct-correctness-fallback"
...
def test_global_direct_fallback_default_excludes_large_local_validation_systems():
    ...
    cfg = fast_global._cfg(Background())
    assert cfg["direct_max_dofs"] < cfg["direct_fallback_max_dofs"]
