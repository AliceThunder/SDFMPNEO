...
def _truth_tensors(background, geometry):
    ...
    if _relative(z, z2) > 1e-12 or _relative(d, d2) > 1e-12:
        raise RuntimeError("final audit truth tensor paths disagree")
    truth = DecodedTensors(z, d, h, d_out, 0.0, 0.0)
    return truth, phi_min, phi_max, {**audit, **port_audit}
...
def _tensor_case(model, geometry):
    ...
    row = {
        "z_relative_error": _relative(predicted.z_field, truth.z_field, z_scale),
    ...
    for current in _current_vectors(truth.z_field.shape[0]):
        ...
        voltage_truth = truth.z_field @ c
        voltage_pred = predicted.z_field @ c
        current_error = max(
            current_error,
            _relative(voltage_pred, voltage_truth, z_scale * np.sqrt(c2)),
        )
        p_truth = truth.volume_power(c)
        p_pred = predicted.volume_power(c)
        current_error = max(
            current_error,
            abs(p_pred - p_truth) / max(abs(p_truth), 1e-10 * d_scale * c2, np.finfo(float).tiny),
        )
        q_truth = truth.modal_heat(c)
        q_pred = predicted.modal_heat(c)
        current_error = max(
            current_error,
            _relative(q_pred, q_truth, h_scale * c2),
        )
    row["maximum_current_space_relative_error"] = float(current_error)
...
def _rhs(model, context, tensors, operating, state):
    ...
    q = model._heat_source_with_tensors(a, context, tensors, operating)[0]
...
def _integrate(model, context, tensors, operating, times):
    ...
    if times.ndim != 1 or times.size == 0 or np.any(times <= 0.0) or np.any(np.diff(times) <= 0.0):
        raise ValueError("final audit times must be strictly increasing and positive")
    initial = np.zeros(model.thermal_rank, float)
    result = solve_ivp(
        lambda t, a: _rhs(model, context, tensors, operating, a),
        (0.0, float(times[-1])),
        initial,
        t_eval=times,
        method="BDF",
    ...
    if not result.success or result.y.shape != (model.thermal_rank, len(times)):
        raise RuntimeError("final audit reduced reference integration failed: " + str(result.message))
...
def _steady(model, context, tensors, operating, initial):
    ...
    if residual_norm <= 1e-10:
        jacobian = _numerical_jacobian(function, state, residual)
...
def _circuit_condition(model, context, tensors, operating, state):
    ...
    total = tensors.z_field + np.diag(resistance) + model._series_impedance(
        operating.get("series_impedance"), len(resistance)
    )
    return float(np.linalg.cond(total))


def _trajectory_case(model, geometry, truth, predicted, operating, times):
    context = model.geometry_context(geometry)
    truth_states = _integrate(model, context, truth, operating, times)
    predicted_states = _integrate(model, context, predicted, operating, times)
    phi = np.asarray(context.thermal_basis, float)
    mass = np.asarray(context.thermal_mass_reduced, float)
    worst = 0.0
    maximum_circuit_condition = 1.0
    rows = []
    for time, a_truth, a_pred in zip(times, truth_states, predicted_states):
        diff = a_pred - a_truth
        denom = float(a_truth @ (mass @ a_truth))
        state_error = float(np.sqrt(max(float(diff @ (mass @ diff)), 0.0) / max(denom, np.finfo(float).tiny)))
        theta_truth = phi @ a_truth
        theta_pred = phi @ a_pred
        scale = max(float(np.max(np.abs(theta_truth))), np.finfo(float).tiny)
        tmin = abs(float(np.min(theta_pred) - np.min(theta_truth))) / scale
        tmax = abs(float(np.max(theta_pred) - np.max(theta_truth))) / scale
        wire_error = 0.0
        for weights in context.line_heat_weights:
            weights = np.asarray(weights, float)
            ref = float(weights @ theta_truth)
            val = float(weights @ theta_pred)
            wire_error = max(wire_error, abs(val - ref) / max(abs(ref), 1e-10 * scale, np.finfo(float).tiny))
        z_truth = truth.z_field + np.diag(model.background.wire_resistances(context, a_truth))
        z_pred = predicted.z_field + np.diag(model.background.wire_resistances(context, a_pred))
        z_error = _relative(z_pred, z_truth, np.linalg.norm(z_truth))
        currents_truth = model._currents(a_truth, context, truth, operating)
        currents_pred = model._currents(a_pred, context, predicted, operating)
        current_error = _relative(currents_pred, currents_truth, np.linalg.norm(currents_truth))
        condition = max(
            _circuit_condition(model, context, truth, operating, a_truth),
            _circuit_condition(model, context, predicted, operating, a_pred),
        )
        maximum_circuit_condition = max(maximum_circuit_condition, condition)
        composite = max(state_error, tmin, tmax, wire_error, z_error, current_error)
        worst = max(worst, composite)
        rows.append({
            "time": float(time),
            "state_mass_relative_error": state_error,
            "minimum_temperature_relative_error": float(tmin),
            "maximum_temperature_relative_error": float(tmax),
            "maximum_wire_temperature_relative_error": float(wire_error),
            "impedance_relative_error": z_error,
            "current_relative_error": current_error,
            "circuit_condition": condition,
            "composite_relative_error": float(composite),
        })

    truth_steady = _steady(model, context, truth, operating, truth_states[-1])
    predicted_steady = _steady(model, context, predicted, operating, predicted_states[-1])
    a_truth, r_truth, stable_truth, alpha_truth = truth_steady
    a_pred, r_pred, stable_pred, alpha_pred = predicted_steady
    theta_truth = phi @ a_truth
    theta_pred = phi @ a_pred
    scale = max(float(np.max(np.abs(theta_truth))), np.finfo(float).tiny)
    steady_error = max(
        _relative(a_pred, a_truth, np.linalg.norm(a_truth)),
        abs(float(np.min(theta_pred) - np.min(theta_truth))) / scale,
        abs(float(np.max(theta_pred) - np.max(theta_truth))) / scale,
    )
    for weights in context.line_heat_weights:
        weights = np.asarray(weights, float)
        ref = float(weights @ theta_truth)
    ...
    return {
        ...
        "maximum_circuit_condition": float(maximum_circuit_condition),
        "truth_steady_residual": r_truth,
        "predicted_steady_residual": r_pred,
        "truth_steady_stable": bool(stable_truth),
        "predicted_steady_stable": bool(stable_pred),
        "truth_steady_spectral_abscissa": alpha_truth,
        "predicted_steady_spectral_abscissa": alpha_pred,
...
def run_final_held_out_audit(settings, model, geometries, monitor=None):
    ...
    if not geometries:
        raise ValueError("final held-out audit needs at least one geometry")
    ...
    tensor_tol = float(cfg.get("tensor_relative_tolerance", 2e-1))
    current_tol = float(cfg.get("current_space_relative_tolerance", 2e-1))
    outward_tol = float(cfg.get("outward_relative_tolerance", 2e-1))
    projection_limit = float(cfg.get("projection_correction_limit", 2e-1))
    ...
    thermal_error, thermal_worst, thermal_diag, audited_times = audit_geometry_aware_thermal_trajectories(
        model.background,
        model.background.thermal_library,
        geometries,
        times=times,
        monitor=monitor,
    )
    ...
    checks = {
        ...
        "decoder_projection_correction_ok": maximum_projection <= projection_limit,
        "reduced_end_to_end_dynamics_ok": maximum_dynamic <= dynamic_tol,
        "steady_state_ok": steady_ok,
        "circuit_conditioning_ok": maximum_circuit <= circuit_limit,
    }
    ...
    return {
        **{key: bool(value) for key, value in checks.items()},
        ...
        "full_vs_rom_audited_times": [float(v) for v in audited_times],
        "maximum_tensor_relative_error": float(maximum_tensor),
        "maximum_current_space_relative_error": float(maximum_current),
        "maximum_outward_relative_error": float(maximum_outward),
        "maximum_projection_correction": float(maximum_projection),
        "maximum_reduced_dynamic_relative_error": float(maximum_dynamic),
        "maximum_circuit_condition": float(maximum_circuit),
        "thresholds": {
            "full_vs_rom_thermal_tolerance": thermal_tol,
            "tensor_relative_tolerance": tensor_tol,
            "current_space_relative_tolerance": current_tol,
            "outward_relative_tolerance": outward_tol,
            "projection_correction_limit": projection_limit,
            "reduced_dynamic_relative_tolerance": dynamic_tol,
            "circuit_condition_limit": circuit_limit,
        },
        "samples": rows,
        "certified": bool(certified),
        "status": "certified" if certified else "final_audit_failed",
