...
def _thermal_state_anchors(rank, coordinate_bound, max_axes=4):
    ...
    if r < 1 or not np.isfinite(bound) or bound < 0.0:
        raise ValueError("invalid automatic thermal-rank state probe settings")
    rows = [np.zeros(r, dtype=float)]
    for i in range(min(r, int(max_axes))):
        plus = np.zeros(r, dtype=float)
        minus = np.zeros(r, dtype=float)
        plus[i] = bound
        minus[i] = -bound
...
def select_rank_from_modal_response_envelope(
    ...
    *,
    ...
    The envelope entry for mode j estimates max |q_j|/lambda_j at the physical
    anchor set. For a partial probe, a boundary-band condition prevents declaring
    convergence while the highest computed modes remain energetic.
    """
    ...
    if (
        ...
        or rtol < 0.0 or atol < 0.0 or safety < 1.0
    ...
    probe = envelope.size
    boundary_width = min(2, probe)
    for rank in range(1, probe + 1):
        tail = float(np.linalg.norm(envelope[rank:]))
        if safety * tail > limit:
            continue
        boundary = 0.0
        if unresolved and rank < probe:
            boundary_start = max(rank, probe - boundary_width)
            boundary = float(np.linalg.norm(envelope[boundary_start:]))
...
def _probe_modal_response_envelope(
    ...
    *,
    ...
    operating = _operating_anchors(
        train["operating_lower"], train["operating_upper"]
    )
    ...
    states = _thermal_state_anchors(
        core.thermal_model.rank, coordinate_bound, max_axes=max_state_axes
    )
    envelope = np.zeros(core.thermal_model.rank, dtype=float)
    zero = states[0]
    if monitor is not None:
        monitor.phase("thermal_rank_selection")

    A0 = problem.operator_sparse(zero).tocsc()
    lu = spla.splu(A0)
    ...
    hot_u = max(operating, key=lambda u: float(np.linalg.norm(u)))
    ...
    return core, envelope, len(operating), len(states)


def automatic_thermal_rank(config_path, config, *, monitor=None):
    """Progressively select a thermal rank from full-order physical EM anchors.

    This default is a finite physics-envelope convergence criterion, not a
    continuous-domain theorem. If a partial rank cannot be resolved before the
    probe cap, selection fails closed to the full discrete thermal space.
    """
    ...
    max_probe_value = truncation.get("max_probe_rank", 32)
    max_probe = n_free if max_probe_value is None else min(n_free, int(max_probe_value))
    max_probe = max(1, max_probe)
    safety = float(truncation.get("source_bound_safety_factor", 2.0))
    boundary_fraction = float(truncation.get("boundary_fraction", 0.25))
    max_state_axes = int(truncation.get("temperature_probe_axes", 4))

    probe = min(n_free, start, max_probe)
    while True:
        _, envelope, n_operating, n_states = _probe_modal_response_envelope(
            tagged,
            config,
            rank=probe,
            coordinate_bound=coordinate_bound,
            max_state_axes=max_state_axes,
            monitor=monitor,
        )
        ...
        report = {
            ...
            "full_dimension": int(n_free),
            "probe_rank": int(probe),
            "relative_tolerance": relative,
            "absolute_tolerance": absolute,
            ...
            "certified_continuous_domain": False,
            "scope": (
                "deterministic full-order EM equilibrium anchors on the reference "
                "geometry; exact residual training and geometry checks remain separate"
            ),
            **details,
        }
        if rank < probe:
        ...
        if probe >= n_free:
            report["selected_rank"] = int(n_free)
            report["fallback"] = "full_discrete_thermal_space"
            return int(n_free), report
        if probe >= max_probe:
            report["selected_rank"] = int(n_free)
...
def _write_temporary_config_same_directory(path, payload):
    handle = NamedTemporaryFile(
        mode="w", encoding="utf-8", prefix=path.stem + ".auto.",
...
def automatic_model_from_config(path, *, monitor=None):
    ...
    if any(strict):
        ...
        compatible_truncation = dict(truncation)
        compatible_truncation.pop("mode", None)
        for key in (
            "relative_tolerance", "absolute_tolerance", "initial_coordinate_bound",
            "probe_start_rank", "max_probe_rank", "source_bound_safety_factor",
            "boundary_fraction", "temperature_probe_axes",
        ):
            compatible_truncation.pop(key, None)
        compatible["thermal_truncation"] = compatible_truncation
        temporary = _write_temporary_config_same_directory(source, compatible)
        try:
        ...
        model.thermal_rank_report = {
            ...
            "certified_continuous_domain": bool(
                certificate is not None and certificate.certified
            ),
        }
    ...
    compatible = dict(config)
    compatible["thermal_rank"] = int(rank)
    compatible["thermal_truncation"] = {
        "initial_coordinate_bound": truncation.get("initial_coordinate_bound", 0.1)
    }
