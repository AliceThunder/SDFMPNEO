"""Automatic thermal-rank selection without runtime patching."""
...
def select_rank_from_modal_response_envelope(steady_response_envelope, *, relative_tolerance, absolute_tolerance=0.0, safety_factor=1.0, boundary_fraction=0.25, unresolved=True):
    ...
    if not np.isfinite(rtol + atol + safety + fraction) or rtol < 0.0 or atol < 0.0 or safety < 1.0 or not 0.0 < fraction <= 1.0:
    ...
    for rank in range(1, envelope.size + 1):
        ...
        if unresolved and rank < envelope.size:
            ...
            if safety * boundary > fraction * max(limit, np.finfo(float).tiny):
                continue
        return rank, {"total_response_norm": total, "resolved_tail_norm": tail, "selection_limit": limit, "boundary_tail_norm": boundary}
    return envelope.size, {"total_response_norm": total, "resolved_tail_norm": 0.0, "selection_limit": limit, "boundary_tail_norm": 0.0}
...
def _probe_modal_response_envelope(tagged, config, *, rank, coordinate_bound, max_state_axes, monitor=None):
    ...
    core = TetrahedralElectroThermalCore.build_nonlinear(mesh, omega=2 * np.pi * float(config["frequency_hz"]), reluctivity_tetra=nu, conductivity_regions=regions, temperature_reference_nodal=np.full(mesh.n_nodes, ambient), constitutive_relative_error_budget=float(config.get("constitutive_relative_error", 1e-8)), rho_cp_tetra=capacity, thermal_conductivity_tetra=conductivity, source_current=np.zeros(mesh.n_edges), thermal_rank=int(rank))
    problem = core.electromagnetic_problem
    ports = SolidTerminalPortSet.build(tagged, problem, config["terminal_pairs"], names=config.get("port_names"))
    ...
    for u in operating:
        ...
        q = np.asarray([np.real(np.vdot(x, problem.loss_operator_sparse(j, zero) @ x)) for j in range(core.thermal_model.rank)], dtype=float)
    ...
    for state in states[1:]:
        ...
        q = np.asarray([np.real(np.vdot(x, problem.loss_operator_sparse(j, state) @ x)) for j in range(core.thermal_model.rank)], dtype=float)
        envelope = np.maximum(envelope, np.abs(q) / lambdas)
    return envelope, len(operating), len(states)


def automatic_thermal_rank(config_path, config, *, monitor=None):
    """Resolve a physics-envelope rank; unresolved capped probes fail closed to full rank."""
    ...
    assembly = mesh.assemble_p1_thermal(rho_cp_tetra=capacity, conductivity_tetra=conductivity, homogeneous_dirichlet_boundary=True)
    n_free = int(assembly.M.shape[0])
    if n_free < 1:
        raise ValueError("thermal boundary treatment produced no free degrees of freedom")
    if n_free == 1:
        return 1, {"method": "automatic_physics_envelope", "selected_rank": 1, "full_dimension": 1, "probe_rank": 1, "certified_continuous_domain": False, "scope": "single free thermal degree of freedom; no truncation"}
    ...
    while True:
        envelope, n_operating, n_states = _probe_modal_response_envelope(tagged, config, rank=probe, coordinate_bound=coordinate_bound, max_state_axes=state_axes, monitor=monitor)
        rank, details = select_rank_from_modal_response_envelope(envelope, relative_tolerance=relative, absolute_tolerance=absolute, safety_factor=safety, boundary_fraction=boundary_fraction, unresolved=probe < n_free)
        report = {"method": "automatic_physics_envelope", "selected_rank": int(rank), "full_dimension": n_free, "probe_rank": probe, "relative_tolerance": relative, "absolute_tolerance": absolute, "source_bound_safety_factor": safety, "initial_coordinate_bound": coordinate_bound, "operating_anchor_count": n_operating, "temperature_anchor_count": n_states, "maximum_modal_steady_response": envelope.tolist(), "certified_continuous_domain": False, "scope": "deterministic full-order EM equilibrium anchors on the reference geometry", **details}
        if rank < probe:
            return int(rank), report
        if probe >= n_free:
            report.update(selected_rank=n_free, fallback="full_discrete_thermal_space")
            return n_free, report
        if probe >= max_probe:
            report.update(selected_rank=n_free, fallback="probe_cap_reached_use_full_discrete_thermal_space")
            return n_free, report
        probe = min(n_free, max_probe, max(probe + 1, 2 * probe))


def resolve_training_bounds(training, rank, truncation):
    ...
    if automatic:
        ...
        raise ValueError(f"thermal rank selected {rank} modes but explicit initial bounds have another dimension")
    return training
...
__all__ = ["automatic_thermal_rank", "resolve_training_bounds", "select_rank_from_modal_response_envelope", "strict_truncation_kwargs"]
