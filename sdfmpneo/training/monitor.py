...
def _session_matches_model(session_dir, target, root):
    for row in reversed(read_jsonl(Path(session_dir) / "metrics.jsonl")):
        for key in ("checkpoint", "model"):
            value = row.get(key)
            if value is None: continue
            try:
                if _resolved_path(value, root) == target: return True
...
def find_resume_sessions(log_root, resume_model, *, root=None):
    ...
    for directory in directories:
        if _session_matches_model(directory, target, root):
            try: stamp = directory.stat().st_mtime_ns
    ...
    try:
        ...
        for value in payload.get("history_sessions", []):
            path = Path(value)
            if path.is_dir(): lineage.append(path)
...
def build_resume_history(log_root, resume_model, *, root=None):
    ...
    revision_offset = 0; elapsed_offset = 0.0; collocation_offset = 0; combined = []
...
class TrainingMonitor:
    ...
    def _read_command(self):
        ...
        try: command = json.loads(self.control_path.read_text(encoding="utf-8"))["command"]
        except (OSError, ValueError, KeyError): return
        if command not in {"run", "pause", "stop"}: return
        with self._lock:
            if self._command == "stop": return
            self._command = command
            if self.data["state"] in {"completed","stopped","failed","stalled"}: return
            if command == "stop": self.data["state"] = "stopping"
            elif command == "pause" and self.data["state"] != "paused": self.data["state"] = "pausing"
            elif command == "run" and self.data["state"] in {"paused","pausing"}: self.data["state"] = "resuming"
    ...
    def checkpoint(self):
        while True:
            ...
            with self._lock:
                ...
                if command == "stop": raise TrainingStopped("用户停止训练")
                if command == "run": self.data["state"] = "running"; return
    ...
    def retain(self, network):
        ...
        with self._lock: self.data["nodes"] = len(network.response_nodes)
    ...
    def finish(self, state, **details):
        with self._lock: self.data.update(state=state, **details)


class JsonlTail:
    def __init__(self, path): self.path = Path(path); self.offset = 0; self.pending = b""
    def read(self):
        try:
            with self.path.open("rb") as source:
                if source.seek(0, 2) < self.offset: self.offset, self.pending = 0, b""
                source.seek(self.offset); chunk = source.read(1024*1024); self.offset = source.tell()
        except FileNotFoundError: return []
        lines = (self.pending + chunk).split(b"\n"); self.pending = lines.pop(); result = []
        for line in lines:
            try:
                row = json.loads(line)
