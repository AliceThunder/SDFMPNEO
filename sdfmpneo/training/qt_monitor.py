...
PHASES={
    ...
    "maxwell_basis":"自动构建 residual-driven Maxwell 公共空间",
    "maxwell_operator_samples":"生成 Maxwell residual 训练算子",
    "neural_training":"训练 Maxwell 神经初解器",
    "saving":"保存统一模型",
}
...
class TrainingWindow(QtWidgets.QMainWindow):
    def __init__(self,runner_path,settings,log_root,options,*,parent=None):
        ...
        self.resize(1180,820); central=QtWidgets.QWidget(); self.setCentralWidget(central); layout=QtWidgets.QVBoxLayout(central); title=QtWidgets.QLabel("统一几何 · 自动降阶 · Residual-Corrected 神经电磁–热求解器"); title.setStyleSheet("font-size:21px;font-weight:600;padding:8px"); layout.addWidget(title)
        ...
        layout.addLayout(buttons); self.status_label=QtWidgets.QLabel("就绪"); self.status_label.setWordWrap(True); layout.addWidget(self.status_label); self.details_label=QtWidgets.QLabel("等待自动构建 thermal / Maxwell 物理公共空间。\n两个 rank 都由真实 residual 自动决定；网络只负责 Maxwell 初解。" ); self.details_label.setWordWrap(True); self.details_label.setStyleSheet("QLabel{background:#f8fafc;border:1px solid #e5e7eb;border-radius:6px;padding:8px}"); layout.addWidget(self.details_label); self.path_label=QtWidgets.QLabel(str(self.log_root)); self.path_label.setWordWrap(True); self.path_label.setTextInteractionFlags(QtCore.Qt.TextInteractionFlag.TextSelectableByMouse); layout.addWidget(self.path_label)
        plots=QtWidgets.QGridLayout(); layout.addLayout(plots,1); definitions=[("Maxwell residual loss","loss",True,[("train","train","#2563eb"),("validation","validation","#16a34a")]),("神经训练进度","%",False,[("progress","epoch progress","#7c3aed")]),("物理空间 / 算子准备","样本数",False,[("samples","已完成物理样本","#0891b2")])]
    ...
    def start_training(self):
        ...
        self._launch()
    def _launch(self):
        self._terminal=None; self._stop_requested=False; self._latest_neural=None; self._latest_row=None
    ...
    def consume(self,rows):
        if not rows: return
        row=rows[-1]; self._latest_row=row; self._current_state=row.get("state","running"); self._current_phase=row.get("phase","starting"); samples=row.get("training_points")
    ...
    def _update_details(self,row):
        m=self._latest_neural; epoch="尚未开始" if m is None else f"{m['epoch']}/{m['total']} ({m['percent']:.1f}%)"; train="--" if m is None else f"{m['train']:.5g}"; val="--" if m is None else f"{m['validation']:.5g}"; samples="--" if row is None or row.get("training_points") is None else str(int(row["training_points"])); elapsed="--" if row is None else f"{float(row.get('elapsed_s') or 0):.1f}s"
        trank="--" if row is None or row.get("thermal_basis_rank") is None else str(int(row["thermal_basis_rank"])); tres="--" if row is None or row.get("thermal_basis_residual") is None else f"{float(row['thermal_basis_residual']):.3e}"
        erank="--" if row is None or row.get("basis_rank") is None else str(int(row["basis_rank"])); eres="--" if row is None or row.get("basis_residual") is None else f"{float(row['basis_residual']):.3e}"
        self.details_label.setText(f"自动 thermal rank={trank}  residual={tres}  |  自动 Maxwell rank={erank}  residual={eres}\n神经训练：epoch {epoch}  |  train residual loss={train}  |  validation residual loss={val}\n物理准备样本：{samples}  |  累计用时：{elapsed}\n最终推理不使用训练域 Gate；每次 Maxwell 解都以真实背景 residual 达标为准。")
