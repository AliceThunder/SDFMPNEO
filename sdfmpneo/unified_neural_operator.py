...
class EdgeMultiscaleConfig:
    ...
    def __post_init__(self):
        if self.width < 8 or self.message_passing_steps < 1:
...
def _spmm_batch(torch, matrix, h):
    """Sparse complex neighbor aggregation for [batch,node,width] real states."""
...
def build_edge_residual_operator(background, config=None):
    """Build the shared-weight sparse Maxwell message-passing solver."""
    import torch
    ...
    class SparseMaxwellResidualSolver(torch.nn.Module):
        ...
        def __init__(self):
            ...
            self.input = torch.nn.Sequential(
                torch.nn.Linear(self.dynamic_dimension + self.static_dimension, cfg.width),
                act(),
            ...
            # Fresh models emit zero correction; training learns the full edge-space update.
        ...
        def forward(self, dynamic, normalized_coupling):
            ...
            for block, norm in zip(self.message_blocks, self.norms):
                ...
                update = block(
                    torch.cat(
                        [h, message.real.to(h.dtype), message.imag.to(h.dtype)],
                        dim=-1,
                    )
                )
                h = norm(h + update)
...
def operator_feature_statistics(A):
    """Build the exact sparse coupling graph once for one Maxwell solve."""
    ...
    coupling = matrix.copy()
    coupling.setdiag(0)
    coupling.eliminate_zeros()
    offdiag_abs_sum = np.asarray(np.abs(coupling).sum(axis=1), float).reshape(-1)
    inverse = np.zeros_like(offdiag_abs_sum)
    mask = offdiag_abs_sum > np.finfo(float).tiny
    inverse[mask] = 1.0 / offdiag_abs_sum[mask]
    ...
    return OperatorGraph(
        ...
        node_features=_operator_node_features(diagonal, row_abs_sum, row_nnz),
    )
...
def residual_features(residual, operator_stats):
    """Normalized node features; D^-1 r is an input scale/reference, not the answer."""
...
def neural_correction(network, A, residual, *, operator_stats=None):
    """Direct full-edge neural correction; Jacobi is used only as a fallback."""
    import torch
    ...
    with torch.no_grad():
        y = network(x, coupling).detach().cpu().double().numpy()
    if y.shape != (features.shape[0], features.shape[1], 2) or np.any(~np.isfinite(y)):
        return jacobi
