...
def audit_open_boundary_domain(settings, background, geometries, monitor=None):
    ...
    expanded = bounds.copy(); expanded[:, 0] -= padding; expanded[:, 1] += padding
    ...
    for index, geometry in enumerate(geometries):
        ...
        rows.append(row)
        print(f"开放边界域扩展 Gate……{index+1}/{len(geometries)}  max={row['maximum_relative_error']:.3e}", flush=True)
    ...
    return {"sample_count":len(rows),"padding":padding.tolist(),"relative_tolerance":tolerance,
            "maximum_relative_error":float(worst),"converged":bool(worst<=tolerance),"samples":rows}
...
def audit_low_frequency_formulation(settings, background, geometries, monitor=None):
    ...
    for index, geometry in enumerate(geometries):
        ...
        row["maximum_relative_error"] = max(v for k, v in row.items() if k.startswith("relative_"))
        rows.append(row)
        print(f"full-wave ↔ MQS formulation Gate……{index+1}/{len(geometries)}  max={row['maximum_relative_error']:.3e}", flush=True)
    worst = max((row["maximum_relative_error"] for row in rows), default=0.0)
    return {"sample_count":len(rows),"relative_tolerance":tolerance,
...
def _interpolate_cell_field(source_background, target_points, values):
    field = np.asarray(values, float).reshape(source_background.nx, source_background.ny, source_background.nz)
    interpolation = RegularGridInterpolator(
        source_background.cell_axes, field, method="linear", bounds_error=False, fill_value=None
    )
...
def _steady_thermal_outputs(background, context, X, sigma, phi=None):
    ...
    wire = np.asarray([float(np.dot(np.asarray(w,float),theta)) for w in context.line_heat_weights], float)
    result = {"theta":theta,"maximum_temperature_rise":float(np.max(theta)),"wire_temperature_rise":wire,"mass":M}
    if phi is not None:
        Mr = phi.T @ (M @ phi)
...
def audit_mesh_convergence(settings, background, geometries, monitor=None):
    """Post-basis refinement: add H_j and thermal outputs to the preflight EM audit."""
    ...
    refined = _background_from_settings(settings, fine_step=refined_fine,
    ...
    for index, geometry in enumerate(geometries):
        ...
        row["maximum_relative_error"] = max(v for k,v in row.items() if k.startswith("relative_"))
        rows.append(row)
        print(f"空间 mesh refinement Gate……{index+1}/{len(geometries)}  max={row['maximum_relative_error']:.3e}",flush=True)
    worst=max((row["maximum_relative_error"] for row in rows),default=0.0)
    return {"sample_count":len(rows),"refinement_factor":factor,"base_fine_step":base_fine,
            "refined_fine_step":refined_fine,"relative_tolerance":tolerance,
            "maximum_relative_error":float(worst),"converged":bool(worst<=tolerance),"samples":rows}
...
def _perturb_geometry(geometry, *, translation=None, yaw=None):
    ...
    mapping = g.to_mapping(); port=min(1,len(mapping["coils"])-1)
    ...
    if yaw is not None:
        for section in ("coils","packages"):
            value=np.asarray(mapping[section][port]["angles"],float); value[2]+=float(yaw)
            mapping[section][port]["angles"]=value.tolist()
    return mapping


def _fraction_vector(context):
    return np.concatenate([np.asarray(context.fractions[name],float) for name in sorted(context.fractions)])
...
def audit_geometry_continuity(settings, background, geometries, monitor=None):
    ...
    translation_step=float(cfg.get("translation_step",1e-4)); angle_step=float(cfg.get("angle_step",1e-3))
    limit=float(cfg.get("relative_change_limit",2e-1)); rows=[]
    for index,geometry in enumerate(geometries):
        if monitor is not None: monitor.checkpoint()
        base=background.validate_geometry(geometry)
        variants={"translation":_perturb_geometry(base,translation=[translation_step,0.0,0.0]),
        ...
        for kind,candidate in variants.items():
            try: candidate=background.validate_geometry(candidate)
            ...
            row={"index":int(index),"perturbation":kind,
                 ...
                 "z_relative_change":_relative(z1,z0),"d_vol_relative_change":_relative(d1,d0),
            ...
            rows.append(row)
            print(f"geometry/transport continuity Gate……{index+1}/{len(geometries)} {kind} max={row['maximum_relative_change']:.3e}",flush=True)
    worst=max((row["maximum_relative_change"] for row in rows),default=float("inf"))
    return {"sample_count":len(rows),"translation_step":translation_step,"angle_step":angle_step,
            "relative_change_limit":limit,"maximum_relative_change":float(worst),
            "converged":bool(rows and worst<=limit),"samples":rows}
...
def run_physics_gate(settings, background, dataset, geometries, *, preflight, monitor=None):
    ...
    if not geometries: raise ValueError("Physics Gate needs at least one held-out geometry")
    ...
    checks={
        ...
        "material_fraction_closure_ok":bool(preflight["material_fraction_closure_ok"] and audit["maximum_material_fraction_closure_error"]<=1e-10),
        "finite_support_source_ok":bool(preflight["finite_support_source_ok"] and audit["source_regularization_available"]>=0.5),
    ...
    return {**{k:bool(v) for k,v in checks.items()},
            "reaction_impedance_convention":"negative_source_reaction","phasor_convention":"peak_exp_plus_iwt",
            ...
            "truth_preflight":preflight,"mesh_convergence":mesh,"geometry_continuity":continuity,
            "certified":bool(certified),"status":"certified" if certified else "physics_gate_failed","audit":audit}


__all__=["audit_geometry_continuity","audit_low_frequency_formulation","audit_mesh_convergence",
         "audit_open_boundary_domain","run_physics_gate"]