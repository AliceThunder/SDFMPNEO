"""Fixed multiscale Cartesian Maxwell/thermal background for arbitrary UWPT geometry.

The topology is independent of geometry. Coils are deposited as thin-wire current
sources and sub-cell thermal material fractions; seawater/package losses remain
true 3-D volume losses on the whole background.
"""
...
def stretched_axis(bounds,core_half,fine_step,growth,max_step,center=0.0):
    ...
    if not lo<c<hi or h<=0 or fine<=0 or growth<1 or max_step<fine: raise ValueError("invalid background axis settings")
...
class FixedMultiscaleBackground:
    def __init__(self,x,y,z,*,frequency_hz,materials,coil_materials,package_materials,seawater_material,thermal_rank,ambient_temperature=293.15):
        ...
        for q in (self.x,self.y,self.z):
            if q.ndim!=1 or len(q)<3 or np.any(np.diff(q)<=0): raise ValueError("background axes must be strictly increasing")
        self.dx=np.diff(self.x); self.dy=np.diff(self.y); self.dz=np.diff(self.z); self.nx,self.ny,self.nz=len(self.dx),len(self.dy),len(self.dz)
        self.frequency_hz=float(frequency_hz); self.omega=2*np.pi*self.frequency_hz; self.ambient_temperature=float(ambient_temperature)
        self.materials={str(k):dict(v) for k,v in materials.items()}; self.coil_materials=tuple(map(str,coil_materials)); self.package_materials=tuple(map(str,package_materials)); self.seawater_material=str(seawater_material)
    ...
    def _build_cells(self):
        ...
        V=self.dx[:,None,None]*self.dy[None,:,None]*self.dz[None,None,:]; self.cell_volumes=V.ravel()

    def _build_edges(self):
        full=[]; lengths=[]; active=[]; maps=[]
        # full tuples are (axis,i,j,k); only tangential PEC boundary edges are removed.
        for axis in range(3):
            amap={};
            if axis==0: ranges=(range(self.nx),range(self.ny+1),range(self.nz+1))
            elif axis==1: ranges=(range(self.nx+1),range(self.ny),range(self.nz+1))
            else: ranges=(range(self.nx+1),range(self.ny+1),range(self.nz))
            for i in ranges[0]:
                for j in ranges[1]:
                    for k in ranges[2]:
                        ...
                        idx=len(full); amap[(i,j,k)]=idx; full.append((axis,i,j,k)); lengths.append((self.dx[i],self.dy[j],self.dz[k])[axis])
            maps.append(amap)
    ...
    def _build_curl(self):
        ...
        for i in range(self.nx):
            for j in range(self.ny):
                for k in range(self.nz+1):
                    add(self.edge_maps[0],(i,j,k),1); add(self.edge_maps[1],(i+1,j,k),1); add(self.edge_maps[0],(i,j+1,k),-1); add(self.edge_maps[1],(i,j,k),-1)
                    ...
                    face+=1
        # xz faces, normal -y; orientation is consistent up to a row sign.
        for i in range(self.nx):
            for j in range(self.ny+1):
                for k in range(self.nz):
                    add(self.edge_maps[0],(i,j,k),1); add(self.edge_maps[2],(i+1,j,k),1); add(self.edge_maps[0],(i,j,k+1),-1); add(self.edge_maps[2],(i,j,k),-1)
        ...
        for i in range(self.nx+1):
            for j in range(self.ny):
                for k in range(self.nz):
                    add(self.edge_maps[1],(i,j,k),1); add(self.edge_maps[2],(i,j+1,k),1); add(self.edge_maps[1],(i,j,k+1),-1); add(self.edge_maps[2],(i,j,k),-1)
    ...
    def _build_reconstruction(self):
        ...
        for axis in range(3):
            ...
            for i in range(self.nx):
                for j in range(self.ny):
                    for k in range(self.nz):
                        c=self._cell_id(i,j,k); keys=((i,j,k),(i,j+1,k),(i,j,k+1),(i,j+1,k+1)) if axis==0 else (((i,j,k),(i+1,j,k),(i,j,k+1),(i+1,j,k+1)) if axis==1 else ((i,j,k),(i+1,j,k),(i,j+1,k),(i+1,j+1,k)))
    ...
    def _thermal_basis(self,rank):
        ...
        lo=np.array([self.x[0],self.y[0],self.z[0]]); hi=np.array([self.x[-1],self.y[-1],self.z[-1]]); xi=(self.cell_centers-lo)/(hi-lo)
        modes=[]; m=1
        while len(modes)<rank:
            candidates=[]
            for a in range(1,m+1):
                for b in range(1,m+1):
                    for c in range(1,m+1): candidates.append((a*a+b*b+c*c,a,b,c))
            candidates.sort(); modes=[q[1:] for q in candidates[:rank]]; m+=1
        Phi=np.column_stack([np.sin(a*np.pi*xi[:,0])*np.sin(b*np.pi*xi[:,1])*np.sin(c*np.pi*xi[:,2]) for a,b,c in modes])
        gram=Phi.T@(self.cell_volumes[:,None]*Phi); L=np.linalg.cholesky(gram); return Phi@np.linalg.inv(L.T)

    def _deposit_line(self,points):
        source=np.zeros(self.n_edges,float); heat=np.zeros(self.n_cells,float)
        for p0,p1 in zip(points[:-1],points[1:]):
            d=p1-p0; length=float(np.linalg.norm(d))
            if length==0: continue
            mid=.5*(p0+p1); i=int(np.clip(np.searchsorted(self.x,mid[0])-1,0,self.nx-1)); j=int(np.clip(np.searchsorted(self.y,mid[1])-1,0,self.ny-1)); k=int(np.clip(np.searchsorted(self.z,mid[2])-1,0,self.nz-1)); heat[self._cell_id(i,j,k)]+=length
            for axis,component in enumerate(d):
                if component==0: continue
                if axis==0: candidates=[(i,j,k),(i,j+1,k),(i,j,k+1),(i,j+1,k+1)]
                elif axis==1: candidates=[(i,j,k),(i+1,j,k),(i,j,k+1),(i+1,j,k+1)]
                else: candidates=[(i,j,k),(i+1,j,k),(i,j+1,k),(i+1,j+1,k)]
    ...
    def geometry_context(self,geometry):
        ...
        for coil,mat in zip(g.coils,self.coil_materials):
            points=coil.centerline(spacing); source,heat=self._deposit_line(points); sources.append(source); heat_weights.append(heat)
            area=coil.conductor_width*coil.conductor_thickness; length=np.sum(np.linalg.norm(np.diff(points,axis=0),axis=1)); volume=area*length; fractions[mat]+=heat*volume/self.cell_volumes
        copper=np.zeros(self.n_cells)
        for mat in self.coil_materials: copper+=fractions[mat]
        copper=np.clip(copper,0,1)
        occupied=copper.copy()
        for package,mat in zip(g.packages,self.package_materials):
            mask=package.contains(self.cell_centers).astype(float); add=mask*(1-occupied); fractions[mat]+=add; occupied+=add
        fractions[self.seawater_material]+=np.clip(1-occupied,0,1)
    ...
    def _temperature_material(self,material,temperature):
        m=self.materials[material]; sigma=float(m.get("electrical_conductivity",0)); alpha=float(m.get("resistivity_temperature_coefficient",0)); tref=float(m.get("reference_temperature",self.ambient_temperature)); return sigma/(1+alpha*(temperature-tref))

    def cell_properties(self,context,state=None,*,em=False):
        T=np.full(self.n_cells,self.ambient_temperature) if state is None else self.ambient_temperature+self.thermal_basis@np.asarray(state,float)
        sigma=np.zeros(self.n_cells); eps=np.zeros(self.n_cells); mu_inv=np.zeros(self.n_cells); k=np.zeros(self.n_cells); cap=np.zeros(self.n_cells)
    ...
    def em_operator(self,context,state):
        sigma,eps,mu_inv,_,_,_=self.cell_properties(context,state,em=True); h2=np.asarray(self.face_cell_hodge@mu_inv).ravel(); hs=np.asarray(self.edge_cell_hodge@sigma).ravel(); he=np.asarray(self.edge_cell_hodge@eps).ravel()
    ...
    def thermal_operator_full(self,fractions):
        ...
        def pair(c1,c2,area,distance):
            kf=2*k[c1]*k[c2]/(k[c1]+k[c2]); g=kf*area/distance; diag[c1]+=g; diag[c2]+=g; rows.extend([c1,c2]); cols.extend([c2,c1]); data.extend([-g,-g])
        for i in range(self.nx):
            for j in range(self.ny):
    ...
    def material_joule_cells(self,context,state,X):
        sigma,_,_,_,_,_=self.cell_properties(context,state,em=True); ex,ey,ez=self.field_components(X); weight=.5*sigma*self.cell_volumes
        return ex,ey,ez,weight
