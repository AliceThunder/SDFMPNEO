...
This module keeps the stored physical matrix unchanged.  It supports both an
ordinary complex128 field and the high/low compensated field used only by the
final local refinement.  On platforms with genuinely extended ``longdouble``
...
from .unified_compensated_field import field_parts
...
def residual_roundoff_diagnostics(A, field, rhs, *, standard_residual=None):
    ...
    matrix = _as_csr(A)
    x_high, x_low = field_parts(field)
    b = np.asarray(rhs, complex).reshape(-1)
    if x_high.shape != (matrix.shape[1],) or b.shape != (matrix.shape[0],):
        raise ValueError("Maxwell residual dimensions do not match")
    if standard_residual is None:
        standard_x = np.asarray(x_high + x_low, complex).reshape(-1)
        standard_residual = np.asarray(b - matrix @ standard_x, complex).reshape(-1)
    else:
        standard_residual = np.asarray(standard_residual, complex).reshape(-1)

    norm_b = max(float(np.linalg.norm(b)), np.finfo(float).tiny)
    # abs(high)+abs(low) is a conservative magnitude for the represented field.
    x_bound = np.abs(x_high) + np.abs(x_low)
    absolute_action = np.asarray(abs(matrix) @ x_bound, float).reshape(-1)
    ...
    numerator = absolute_action + np.abs(b)
    denominator = np.maximum(np.abs(standard_residual), np.finfo(float).tiny)
    with np.errstate(over="ignore", invalid="ignore", divide="ignore"):
        cancellation = numerator / denominator
...
def _extended_csr_residual(matrix, x_high, x_low, b, *, rows_per_chunk=8192):
    """Form b-A(high+low) with platform long-double precision when available."""
    n = matrix.shape[0]
    out = np.empty(n, dtype=np.complex128)
    indptr = matrix.indptr
    indices = matrix.indices
    data = matrix.data
    x_ext = np.asarray(x_high, dtype=np.clongdouble) + np.asarray(
        x_low, dtype=np.clongdouble
    )
...
def _complex_product_expansion(data, x):
...
def _compensated_csr_residual(matrix, x_high, x_low, b, *, rows_per_chunk=4096):
    """Portable compensated residual of b-A(high+low) using binary64 expansions."""
    n = matrix.shape[0]
    out = np.empty(n, dtype=np.complex128)
    indptr = matrix.indptr
    indices = matrix.indices
    data = np.asarray(matrix.data, dtype=np.complex128)
    x_high = np.asarray(x_high, dtype=np.complex128)
    x_low = np.asarray(x_low, dtype=np.complex128)
    b = np.asarray(b, dtype=np.complex128)
    has_low = bool(np.any(x_low.real != 0.0) or np.any(x_low.imag != 0.0))
    ...
    for r0 in range(0, n, int(rows_per_chunk)):
        ...
        local_indices = np.asarray(indices[p0:p1], dtype=np.intp)
        high_terms = _complex_product_expansion(local_data, x_high[local_indices])
        if has_low:
            low_terms = _complex_product_expansion(local_data, x_low[local_indices])
        else:
            low_terms = None
        ...
        for local_row in range(r1 - r0):
            ...
            il = 0.0
            for q in range(a, z):
                rh, rl = _dd_add_scalar(rh, rl, -high_terms[0][q], -high_terms[1][q])
                ih, il = _dd_add_scalar(ih, il, -high_terms[2][q], -high_terms[3][q])
                if low_terms is not None:
                    rh, rl = _dd_add_scalar(rh, rl, -low_terms[0][q], -low_terms[1][q])
                    ih, il = _dd_add_scalar(ih, il, -low_terms[2][q], -low_terms[3][q])
...
def accurate_residual_vector(A, field, rhs, *, target_relative=1e-9):
    ...
    b = np.asarray(rhs, complex).reshape(-1)
    standard_x = np.asarray(x_high + x_low, complex).reshape(-1)
    standard = np.asarray(b - matrix @ standard_x, complex).reshape(-1)
    diagnostics = residual_roundoff_diagnostics(
        matrix, field, b, standard_residual=standard
    )

    has_low = bool(np.any(x_low.real != 0.0) or np.any(x_low.imag != 0.0))
    # If the conservative rounding scale is far below the requested certificate
    # and the field is a plain complex128 vector, ordinary CSR is sufficient.
    target = max(float(target_relative), np.finfo(float).tiny)
    if (
        not has_low
        and diagnostics["roundoff_bound_relative"] <= 0.05 * target
    ):
    ...
    if _extended_longdouble_available():
        residual = _extended_csr_residual(matrix, x_high, x_low, b)
        mode = "extended-longdouble-csr"
    else:
        residual = _compensated_csr_residual(matrix, x_high, x_low, b)
