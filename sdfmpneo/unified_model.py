...
class UnifiedNeuralElectroThermalModel:
            ...
            if isinstance(rules, dict) and isinstance(candidate[section], dict):
                for key, rule in rules.items():
                    if key not in candidate[section]:
                        raise ValueError(
                            f"geometry is missing production-domain field {section}.{key}"
                        )
    ...
    def geometry_context(self, geometry=None):
        ...
        if (
            not hasattr(context, "thermal_basis")
            or context.thermal_basis.shape[1] != self.thermal_rank
        ):
    ...
    def _modal_bounds(self, context):
        """Use exactly the same conductivity-loss support as truth label generation."""
        phi = np.asarray(context.thermal_basis, float)
        sigma = np.asarray(
            self.background.cell_properties(context, None, em=True)[0], float
        ).reshape(-1)
    ...
    def tensors(self, geometry=None):
        ...
        phi_min, phi_max = self._modal_bounds(context)
        value = self.surrogate.predict(context.geometry, phi_min, phi_max)
    ...
    def heat_source(self, state, context, operating):
        return self._heat_source_with_tensors(
            state, context, self.tensors(context.geometry), operating
        )
    ...
    def evaluate(self, state, geometry, operating):
        ...
        derivative = np.linalg.solve(
            operator.mass, -operator.stiffness @ a + q
        )
        return {
            "heat_source": q,
            "derivative": derivative,
            "temperature": self.temperature_field(a, context=context),
            "maximum_temperature": self.maximum_temperature(a, context=context),
    ...
    def _numerical_jacobian(function, state, value=None):
        ...
        for k in range(a.size):
            ...
            trial[k] += epsilon[k]
            jacobian[:, k] = (
                np.asarray(function(trial), float).reshape(-1) - base
            ) / epsilon[k]
        return jacobian
    ...
    def steady_state(
        ...
        *,
        ...
        if converged:
            ...
            stable = bool(
                np.isfinite(spectral_abscissa) and spectral_abscissa < 0.0
            )
        else:
    ...
    def save(self, path, *, metadata=None):
        ...
        library = self.background.thermal_library
        meta = {
            ...
            "thermal_reference_geometry": library.reference_geometry.to_mapping(),
            "thermal_time_scales": list(library.time_scales),
            "thermal_conditioning_limit": float(library.conditioning_limit),
            "thermal_local_count": len(library.local_modes),
            "modal_bound_support": "physical_conductivity_loss_support",
        ...
        arrays = {
            ...
            "thermal_background_modes": library.background_modes,
        ...
        for p, modes in enumerate(library.local_modes):
            arrays[f"thermal_local_modes_{p}"] = modes
        for key, value in checkpoint["network_state"].items():
            arrays["network__" + key.replace(".", "__DOT__")] = (
    ...
    def load(cls, path, *, device="cpu"):
        ...
        with np.load(path, allow_pickle=False) as data:
            ...
            if (
                ...
                or int(meta.get("format_version", -1)) != FORMAT_VERSION
            ):
                raise ValueError(
                    "model is not the current geometry-aware thermal tensor architecture"
                )
            if meta.get("modal_bound_support") != "physical_conductivity_loss_support":
                raise ValueError("model modal-bound convention is incompatible")
            library = GeometryAwareThermalLibrary(
                UnifiedUWPTGeometry.from_mapping(
                    meta["thermal_reference_geometry"]
                ),
                np.asarray(data["thermal_background_modes"], float),
                tuple(
                    np.asarray(data[f"thermal_local_modes_{p}"], float)
                    for p in range(int(meta["thermal_local_count"]))
                ),
            ...
            for name in data.files:
                if name.startswith("network__"):
                    ...
                    state[key] = torch.as_tensor(
                        data[name], dtype=dtype, device=device
                    )
