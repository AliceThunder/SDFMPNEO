...
class _Field:
    ...
    def heat_source(self,state,_geometry,_operating): return self.model._heat_source_with_tensors(state,self.context,self.tensors,self.operating)[0]
...
class UnifiedNeuralElectroThermalModel:
    def __init__(self,background,surrogate,*,default_geometry,current_offset=None,current_matrix=None,production_domain=None,context_cache_size=16):
        if getattr(background,"thermal_library",None) is None or background.thermal_rank<1: raise ValueError("unified model requires a geometry-aware thermal library")
        if surrogate.thermal_rank!=background.thermal_rank: raise ValueError("tensor surrogate and geometry-aware thermal ranks differ")
    ...
    def geometry_context(self,geometry=None):
        ...
        else: self._validate_production_geometry(mapping); g=UnifiedUWPTGeometry.from_mapping(mapping)
        ...
        if not hasattr(context,"thermal_basis") or context.thermal_basis.shape[1]!=self.thermal_rank: raise RuntimeError("geometry-specific thermal basis was not assembled")
        self._contexts[key]=context
        if len(self._contexts)>self.context_cache_size: self._contexts.popitem(last=False)
        return context

    def _modal_bounds(self,context):
        phi=np.asarray(context.thermal_basis,float)
        sigma=np.asarray(self.background.cell_properties(context,None,em=True)[0],float).reshape(-1)
        threshold=max(float(np.max(sigma)),1.0)*1e-14; support=sigma>threshold
        if not np.any(support): raise RuntimeError("physical conductivity-loss support is empty")
    ...
    def tensors(self,geometry=None):
        ...
        if key in self._tensor_cache: self._tensor_cache.move_to_end(key); return self._tensor_cache[key]
        phi_min,phi_max=self._modal_bounds(context); value=self.surrogate.predict(context.geometry,phi_min,phi_max)
    ...
    def project_initial_temperature(self,rise,geometry=None):
        ...
        if value.shape!=(self.background.n_cells,) or np.any(~np.isfinite(value)): raise ValueError("initial temperature rise must be scalar or one value per cell")
    ...
    def temperature_field(self,state,geometry=None,*,context=None):
        context=self.geometry_context(geometry) if context is None else context; a=np.asarray(state,float).reshape(-1)
    ...
    def maximum_temperature(self,state,geometry=None,*,context=None):
        field=self.temperature_field(state,geometry,context=context); return float(max(self.background.ambient_temperature,np.max(field)))
    ...
    def heat_source(self,state,context,operating): return self._heat_source_with_tensors(state,context,self.tensors(context.geometry),operating)
    ...
    def evaluate(self,state,geometry,operating):
        ...
        return {"heat_source":q,"derivative":derivative,"temperature":self.temperature_field(a,context=context),
    ...
    def predict(self,time,*,initial_state,geometry,operating,max_step=100.0,method="etd2_adaptive",rtol=1e-5,atol=1e-8,initial_step=None):
        ...
        return UnifiedPrediction(time=t,state=result.state,derivative=e["derivative"],heat_source=e["heat_source"],maximum_temperature=e["maximum_temperature"],
                                 impedance=e["impedance"],currents=e["currents"],volume_power=e["volume_power"],wire_power=e["wire_power"],outward_power=e["outward_power"],
                                 tensor_projection_correction=e["tensor_projection_correction"],zd_projection_correction=e["zd_projection_correction"],h_projection_correction=e["h_projection_correction"],
    ...
    def steady_state(self,*,initial_guess,geometry,operating,tolerance=1e-10,max_iterations=40):
        ...
        return UnifiedSteadyState(state=state,residual_norm=residual_norm,iterations=last_iteration,converged=converged,stable=stable,spectral_abscissa=spectral_abscissa,
                                  maximum_temperature=e["maximum_temperature"],impedance=e["impedance"],currents=e["currents"],volume_power=e["volume_power"],wire_power=e["wire_power"],outward_power=e["outward_power"],
                                  tensor_projection_correction=e["tensor_projection_correction"],zd_projection_correction=e["zd_projection_correction"],h_projection_correction=e["h_projection_correction"])
    ...
    def save(self,path,*,metadata=None):
        ...
        meta={"format_version":FORMAT_VERSION,"architecture":ARCHITECTURE,"frequency_hz":self.background.frequency_hz,"ambient_temperature":self.background.ambient_temperature,
              "materials":_plain(self.background.materials),"coil_materials":list(self.background.coil_materials),"package_materials":list(self.background.package_materials),
              "seawater_material":self.background.seawater_material,"thermal_rank":self.thermal_rank,"thermal_reference_geometry":library.reference_geometry.to_mapping(),
              "thermal_time_scales":list(library.time_scales),"thermal_conditioning_limit":float(library.conditioning_limit),"thermal_local_count":len(library.local_modes),
              "modal_bound_support":"physical_conductivity_loss_support","default_geometry":_plain(self.default_geometry),"production_domain":_plain(self.production_domain),
              "network_config":checkpoint["network_config"],"network_dtype":checkpoint["dtype"],"n_ports":checkpoint["n_ports"],"metadata":_plain(dict(metadata or {}))}
        arrays={"metadata_json":np.array(json.dumps(meta,sort_keys=True,allow_nan=False)),"background_x":self.background.x,"background_y":self.background.y,"background_z":self.background.z,
                "thermal_background_modes":library.background_modes,"current_offset":self.current_offset,"current_matrix":self.current_matrix,"input_mean":checkpoint["input_mean"],
                "input_scale":checkpoint["input_scale"],"output_mean":checkpoint["output_mean"],"output_scale":checkpoint["output_scale"],"pod_basis":checkpoint["pod_basis"]}
    ...
    def load(cls,path,*,device="cpu"):
        ...
        with np.load(path,allow_pickle=False) as data:
            ...
            if meta.get("modal_bound_support")!="physical_conductivity_loss_support": raise ValueError("model modal-bound convention is incompatible")
            library=GeometryAwareThermalLibrary(UnifiedUWPTGeometry.from_mapping(meta["thermal_reference_geometry"]),np.asarray(data["thermal_background_modes"],float),
                                                tuple(np.asarray(data[f"thermal_local_modes_{p}"],float) for p in range(int(meta["thermal_local_count"]))),
                                                tuple(float(v) for v in meta["thermal_time_scales"]),float(meta.get("thermal_conditioning_limit",1e10)))
            background=OpenBoundaryBackground(data["background_x"],data["background_y"],data["background_z"],frequency_hz=meta["frequency_hz"],materials=meta["materials"],
            ...
            background.set_thermal_library(library); config=ResidualMLPConfig(**dict(meta["network_config"]))
            normalizer=FeatureNormalizer(np.asarray(data["input_mean"],float),np.asarray(data["input_scale"],float)); network=build_residual_mlp(config,normalizer)
            dtype=torch.float32 if meta.get("network_dtype")=="float32" else torch.float64; network=network.to(device=device,dtype=dtype); state={}
            for name in data.files:
                if name.startswith("network__"): state[name[len("network__"):].replace("__DOT__",".")]=torch.as_tensor(data[name],dtype=dtype,device=device)
            ...
            return cls(background,surrogate,default_geometry=meta["default_geometry"],current_offset=np.asarray(data["current_offset"],complex),
                       current_matrix=np.asarray(data["current_matrix"],complex),production_domain=meta.get("production_domain"))
