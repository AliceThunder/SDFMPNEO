...
def train_maxwell_accelerator(background, dataset, *, network_settings=None, training_settings=None,
    ...
    def evaluate(ids):
        ...
        with torch.no_grad():
            for idx in ids:
                _, final_loss = system_losses(int(idx)); values.append(float(final_loss.detach().cpu()))
    ...
    for epoch in range(start, cfg.epochs):
        ...
        if validate:
            ...
            if not np.isfinite(patience_reference): patience_reference = last_validation; stale = 0
                ...
                if last_validation <= patience_reference - required: patience_reference = last_validation; stale = 0
