...
def residual_loss(torch,y,base,scale,Q,S,N,p,r):
    ...
    return torch.mean(residual2/N.clamp_min(torch.finfo(c.dtype).tiny))
...
def train_maxwell_accelerator(dataset,*,network_settings=None,training_settings=None,device="cuda",monitor=None,checkpoint_path=None):
    ...
    if min(len(train_ids),len(val_ids),len(test_ids))<1: raise ValueError("operator dataset must contain train/validation/test samples")
    normalizer=FeatureNormalizer.fit(dataset.features[train_ids]); ns=dict(network_settings or {}); netcfg=ResidualMLPConfig(input_dimension=dataset.features.shape[1],output_dimension=2*r*p,**ns); model=build_residual_mlp(netcfg,normalizer)
    actual=str(device)
    ...
    tensors=[torch.as_tensor(x,dtype=dtype,device=actual) for x in (dataset.features,dataset.baseline,dataset.coefficient_scale,dataset.residual_gram,dataset.residual_linear,dataset.rhs_norm2)]; F,B0,CS,Q,S,N=tensors
    checkpoint=None if checkpoint_path is None else Path(checkpoint_path); start=0; best=float("inf"); best_epoch=0; best_state=copy.deepcopy(model.state_dict()); checkpoint_identity={"network_config":netcfg.to_dict(),"training_config":cfg.to_dict(),"feature_dimension":int(dataset.features.shape[1]),"reduced_rank":int(r),"n_rhs":int(p)}
    if checkpoint is not None and checkpoint.is_file():
        try:
            try: saved=torch.load(checkpoint,map_location=actual,weights_only=False)
            except TypeError: saved=torch.load(checkpoint,map_location=actual)
            if saved.get("identity")!=checkpoint_identity: raise ValueError("training configuration or physical feature dimensions changed")
            model.load_state_dict(saved["network"],strict=True); opt.load_state_dict(saved["optimizer"]); start=int(saved["epoch"]); best=float(saved["best"]); best_epoch=int(saved["best_epoch"]); best_state=saved["best_state"]; print(f"恢复 Maxwell residual 训练：epoch={start}",flush=True)
    ...
    for epoch in range(start,cfg.epochs):
        ...
        last_train=total/max(count,1); completed=epoch+1; validate=completed==1 or completed%cfg.validation_interval==0 or completed==cfg.epochs
        if validate:
            last_val=evaluate(val_ids); threshold=best-1e-10*max(1.0,abs(best)) if np.isfinite(best) else float("inf")
        ...
        if checkpoint is not None:
            checkpoint.parent.mkdir(parents=True,exist_ok=True); temporary=checkpoint.with_suffix(checkpoint.suffix+".tmp"); torch.save({"identity":checkpoint_identity,"epoch":completed,"network":model.state_dict(),"optimizer":opt.state_dict(),"best":best,"best_epoch":best_epoch,"best_state":best_state},temporary); temporary.replace(checkpoint)
        if stale>=cfg.patience: break
    model.load_state_dict(best_state); model.eval(); test=evaluate(test_ids)
    report=MaxwellTrainingReport(completed,best_epoch,float(best),float(last_train),float(last_val),float(test),completed<cfg.epochs,actual,cfg.to_dict(),netcfg.to_dict())
    return model,report
