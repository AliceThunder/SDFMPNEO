...
class Pose:
    ...
    def __post_init__(self):
        object.__setattr__(self,"translation",_v3(self.translation,"translation")); object.__setattr__(self,"angles",_v3(self.angles,"angles"))
    @property
    def rotation(self):
        r,p,y=self.angles; cr,sr=np.cos(r),np.sin(r); cp,sp=np.cos(p),np.sin(p); cy,sy=np.cos(y),np.sin(y)
        Rx=np.array([[1,0,0],[0,cr,-sr],[0,sr,cr]],float); Ry=np.array([[cp,0,sp],[0,1,0],[-sp,0,cp]],float); Rz=np.array([[cy,-sy,0],[sy,cy,0],[0,0,1]],float)
        return Rz@Ry@Rx
...
@dataclass(frozen=True)
class CoilGeometry:
    name: str; shape: str; conductor_width: float; conductor_thickness: float; pose: Pose
    turns: float=1.; outer_half_size: float=.02; pitch: float=.002; corner_radius: float|None=None; control_points: np.ndarray|None=None
    def __post_init__(self):
        ...
        if self.shape in {"circle","rounded_square"}:
            ...
            if self.outer_half_size-self.pitch*self.turns<=.5*self.conductor_width: raise ValueError("spiral collapses")
            if self.shape=="rounded_square" and (self.corner_radius is None or self.corner_radius<=self.pitch*self.turns+.5*self.conductor_width or self.corner_radius>=self.outer_half_size): raise ValueError("invalid corner_radius")
        else:
            q=np.asarray(self.control_points,float)
            if q.ndim!=2 or q.shape[1]!=3 or len(q)<2 or np.any(~np.isfinite(q)): raise ValueError("custom coil needs finite control_points (n>=2,3)")
            object.__setattr__(self,"control_points",q)
    @classmethod
    def from_mapping(cls,name,m):
        d=dict(m); pose=Pose(d.pop("translation",[0,0,0]),d.pop("angles",[0,0,0])); return cls(name=name,pose=pose,**d)
    @staticmethod
    def _rounded(theta,half,radius):
        c=np.abs(np.cos(theta)); s=np.abs(np.sin(theta)); center=half-radius; tiny=np.finfo(float).tiny
        tv=half/np.maximum(c,tiny); th=half/np.maximum(s,tiny); b=center*(c+s); disc=np.maximum(b*b-(2*center*center-radius*radius),0)
    ...
    def centerline(self,spacing):
        ...
        if self.shape in {"polyline","spline"}:
            ...
            else: local=np.column_stack([np.interp(s,dist,q[:,k]) for k in range(3)])
            return self.pose.apply(local)
        end=2*np.pi*self.turns; length=max(2*np.pi*self.outer_half_size*self.turns,spacing); n=max(16,int(np.ceil(length/spacing))+1); th=np.linspace(0,end,n)
        off=self.pitch*th/(2*np.pi); half=self.outer_half_size-off; radial=half if self.shape=="circle" else self._rounded(th,half,float(self.corner_radius)-off)
        return self.pose.apply(np.column_stack([radial*np.cos(th),radial*np.sin(th),np.zeros_like(th)]))
    def to_mapping(self):
        d={"shape":self.shape,"turns":float(self.turns),"outer_half_size":float(self.outer_half_size),"pitch":float(self.pitch),"conductor_width":float(self.conductor_width),"conductor_thickness":float(self.conductor_thickness),"corner_radius":None if self.corner_radius is None else float(self.corner_radius),"translation":self.pose.translation.tolist(),"angles":self.pose.angles.tolist()}
        if self.control_points is not None: d["control_points"]=np.asarray(self.control_points).tolist()
        return d

@dataclass(frozen=True)
class PackageGeometry:
    half_extent: np.ndarray; pose: Pose
    def __post_init__(self):
    ...
    def to_mapping(self): return {"half_extent":self.half_extent.tolist(),"translation":self.pose.translation.tolist(),"angles":self.pose.angles.tolist()}

@dataclass(frozen=True)
class UnifiedUWPTGeometry:
    coils: tuple; packages: tuple
    def __post_init__(self):
    ...
    def from_mapping(cls,m):
        d=dict(m)
        if "coils" in d:
            coils=tuple(CoilGeometry.from_mapping(str(v.get("name",f"port_{i}")),{k:x for k,x in v.items() if k!="name"}) for i,v in enumerate(d["coils"])); pv=list(d.get("packages",[]))
            if len(pv)!=len(coils): raise ValueError("packages must match coils")
            packages=tuple(PackageGeometry(v["half_extent"],Pose(v.get("translation",c.pose.translation),v.get("angles",c.pose.angles))) for v,c in zip(pv,coils)); return cls(coils,packages)
        tx=CoilGeometry.from_mapping("tx",d["transmitter"]); rx=CoilGeometry.from_mapping("rx",d["receiver"]); common=d.get("package_half_extent",[.02,.02,.004])
        return cls((tx,rx),(PackageGeometry(d.get("tx_package_half_extent",common),tx.pose),PackageGeometry(d.get("rx_package_half_extent",common),rx.pose)))
    def to_mapping(self): return {"coils":[dict(name=c.name,**c.to_mapping()) for c in self.coils],"packages":[p.to_mapping() for p in self.packages]}
...
def _sample(value,rule,rng):
    ...
    if "choices" in rule:
        q=list(rule["choices"]); return copy.deepcopy(q[int(rng.integers(len(q)))])
...
def sample_geometry(base,sampling,rng):
    ...
    for section,rules in dict(sampling or {}).items():
        ...
        elif section in out: out[section]=_sample(out[section],rules,rng)
    return out
