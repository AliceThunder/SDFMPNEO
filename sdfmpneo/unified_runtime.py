...
def build_background(settings):
    ...
    return FixedMultiscaleBackground.from_config(
        settings["BACKGROUND"],frequency_hz=settings["PHYSICS"]["frequency_hz"],
        materials=settings["MATERIALS"],coil_materials=r["coil_materials"],
        package_materials=r["package_materials"],seawater_material=r["seawater_material"],
        thermal_rank=settings["THERMAL_RANK"],ambient_temperature=settings["PHYSICS"]["ambient_temperature"],
    )
...
def train(settings,model_path,settings_dir,monitor=None):
    ...
    sig=_signature(settings); meta_path,basis_path,data_path=_cache_paths(settings_dir)
    ...
    try:
        ...
        network,report=train_maxwell_accelerator(
            dataset,network_settings=settings["TRAINING"].get("network"),
        ...
        accelerator=NeuralMaxwellAccelerator(
            network,V,residual_tolerance=float(settings["PHYSICS"].get("maxwell_residual_tolerance",1e-7)),
            max_iterations=int(settings["PHYSICS"].get("maxwell_max_iterations",200)),
        )
        model=UnifiedNeuralElectroThermalModel(
            bg,accelerator,default_geometry=settings["DEFAULT_GEOMETRY"],
            current_offset=settings["PORTS"].get("current_offset"),current_matrix=settings["PORTS"].get("current_matrix"),
        )
        if monitor is not None: monitor.phase("saving",check=False)
        _progress("保存统一神经物理模型",98,monitor)
        model.save(model_path,metadata={"basis_report":basis_report,"training_report":asdict(report)})
        checkpoint.unlink(missing_ok=True)
        write_json(settings_dir/"training.report.json",{
            "model":str(model_path),"background_cells":bg.n_cells,"maxwell_dofs":bg.n_edges,
            "em_basis_rank":V.shape[1],"basis":basis_report,"training":report,
        })
        ...
        print(f"训练已停止；神经训练检查点：{checkpoint}" if checkpoint.is_file() else "训练已停止。",flush=True)
        return 130
...
def predict(settings,model_path,output_path,settings_dir):
    ...
    initial=np.asarray(p["a0"],float); operating=np.asarray(p["operating"],float); results=[]
    ...
    for requested in p["times"]:
        if isinstance(requested,str) and requested.lower()=="inf":
            r=model.steady_state(
                initial_guess=initial,geometry=geometry,operating=operating,
                tolerance=float(p.get("steady_tolerance",1e-10)),max_iterations=int(p.get("steady_max_iterations",40)),
            )
            print(f"t=inf，Tmax={r.maximum_temperature:.6g} K，thermal residual={r.residual_norm:.3e}，Maxwell residual={max(r.maxwell_final_residual):.3e}",flush=True)
            results.append({"time":"inf","steady_state":r})
        else:
            t=float(requested)
            r=model.predict(
                t,initial_state=initial,geometry=geometry,operating=operating,
                max_step=float(p.get("max_step",100)),method=p.get("method","etd2_adaptive"),
                rtol=float(p.get("rtol",1e-5)),atol=float(p.get("atol",1e-8)),initial_step=p.get("initial_step"),
            )
            print(
                f"t={t:g}s，Tmax={r.maximum_temperature:.6g} K，steps={r.steps}，"
                f"Maxwell initial={max(r.maxwell_initial_residual):.3e} → final={max(r.maxwell_final_residual):.3e}，"
                f"correction iterations={max(r.maxwell_correction_iterations)}",
                flush=True,
            )
            results.append({"time":t,"prediction":r})
    write_json(output_path,{"model":str(model_path),"geometry":geometry,"operating":operating,"results":results})
...
def launch(settings,argv=None):
    ...
    if args.mode=="predict":
        out=Path(settings["FILES"]["predictions"]); out=out if out.is_absolute() else root/out
