...
def train(settings,model_path,settings_dir,monitor=None):
    ...
    try:
        ...
        surrogate,report=train_matrix_tensor_surrogate(dataset,network_settings=settings["TRAINING"].get("network"),training_settings=settings["TRAINING"].get("optimizer"),
            device=settings["TRAINING"].get("device","cuda"),monitor=monitor,checkpoint_path=checkpoint)
        model=UnifiedNeuralElectroThermalModel(bg,surrogate,default_geometry=settings["DEFAULT_GEOMETRY"],current_offset=settings["PORTS"].get("current_offset"),
            current_matrix=settings["PORTS"].get("current_matrix"),production_domain=settings.get("GEOMETRY_SAMPLING"))
        final_cfg=dict(settings["TRAINING"].get("final_audit",{})); final_count=max(1,int(final_cfg.get("samples",2)))
        final_rng=np.random.default_rng(seed+524287); final_geometries=_sample_geometries(settings,final_count,final_rng,bg)
        _progress("执行 completely-held-out final Go/No-Go audit",92,monitor); final_audit=run_final_held_out_audit(settings,model,final_geometries,monitor=monitor)
        if not final_audit["certified"]: raise RuntimeError("Final held-out audit failed; refusing model artifact: "+json.dumps(jsonable(final_audit),sort_keys=True))
        ...
        model.save(model_path,metadata={"thermal_basis_report":thermal_report,"truth_preflight":preflight,"physics_gate":gate,"final_held_out_audit":final_audit,"training_report":asdict(report)})
        checkpoint.unlink(missing_ok=True)
        write_json(settings_dir/"training.report.json",{"model":str(model_path),"background_cells":bg.n_cells,"offline_maxwell_dofs":bg.n_edges,
            ...
            "thermal_representation":"deterministic Phi(g) = BG + transported local blocks","thermal_basis":thermal_report,"truth_preflight":preflight,"physics_gate":gate,
            "final_held_out_audit":final_audit,"training":report})
