"""Finite-horizon fixed-network training -> save -> reload -> rollout -> independent check."""
...
def main():
    ...
    # 0.8 s is longer than the 0.25 s learned segment; predict() rolls out automatically.
    prediction = model.predict(0.8, a0=[1.0] * rank, operating=[2000.0, 500.0])
    validation = model.validate_trajectory(
        [0.0, 0.05, 0.1, 0.25, 0.5, 0.8],
        a0=[1.0] * rank,
        operating=[2000.0, 500.0],
    )
    ...
    print("rollout segments:", prediction["segment_durations"])
    print("independent maximum temperature error (K):", validation["maximum_temperature_error"])
    if not report.numerical_tolerance_met:
        raise SystemExit("training did not reach the requested residual/restart tolerance; inspect results.json")
